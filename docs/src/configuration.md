...
| `animated_covers` | bool | `false` | Embed an animated WebP front cover plus the current static JPEG as a managed fallback for clips with a video preview. If the advertised JPEG cannot be fetched, the file is left untouched and retried later. |
| `video_cover_retention` | `neither` \| `webp` \| `mp4` \| `both` | `neither` | Unified control for the animated cover. `webp` embeds the animated WebP front cover plus the static JPEG fallback, `mp4` keeps the raw album `cover.mp4` (the `video_cover_url` byte-for-byte, no transcode), `both` does both, `neither` neither. Overrides `animated_covers` when set. The standalone music video (`video_url`) is a separate asset with its own `video_mp4` toggle. |
