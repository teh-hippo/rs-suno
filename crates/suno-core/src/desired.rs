    ...
    clips
        ...
        .map(|((clip, name), lineage)| {
            ...
            Desired {
                ...
                // (`apply_synced_lrc`, in every run mode) before reconcile; the
...
/// Retarget non-owned lossless clips to native MP3 while preserving existing lossless files.
pub fn apply_non_owned_audio_policy(
    desired: &mut [Desired],
    manifest: &Manifest,
    ...
    for d in desired {
        if !d.format.requires_wav_render()
            || d.clip.user_id.is_empty()
            || d.clip.user_id == authenticated_user_id
        {
            continue;
        }
